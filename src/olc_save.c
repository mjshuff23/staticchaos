...
/*
 * function to save the mobprogs of one mobile, called
 * from within save_mobiles();
 */
void save_mobprogs( FILE *fp, MOB_INDEX_DATA *mob )
{
  MPROG_DATA *mprog;

  if ( mob == NULL || mob->mobprogs == NULL )
    return;

  for ( mprog = mob->mobprogs; mprog != NULL; mprog = mprog->next )
  {
    fprintf( fp, ">%s ", mprog_type_to_name( mprog->type ) );
    fprintf( fp, "%s~\n", mprog->arglist );
    fprintf( fp, "%s~\n", mprog->comlist );
  }
  fprintf( fp, "|\n" );
