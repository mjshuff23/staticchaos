...
RESET_DATA *new_reset_data( void )
{
    ...
    if ( !reset_free )
    {
	pReset		= alloc_perm( sizeof(*pReset), PERM_OLC );
	top_reset++;
    }
    else
    {
...
AREA_DATA *new_area( void )
{
    ...
    if ( !area_free )
    {
	pArea		= alloc_perm( sizeof(*pArea), PERM_OLC );
	top_area++;
    }
    else
    {
...
EXIT_DATA *new_exit( void )
{
    ...
    if ( !exit_free )
    {
        pExit           =   alloc_perm( sizeof(*pExit), PERM_OLC );
        top_exit++;
    }
    else
    {
...
EXTRA_DESCR_DATA *new_extra_descr( void )
{
    ...
    if ( !extra_descr_free )
    {
        pExtra              =   alloc_perm( sizeof(*pExtra), PERM_OLC );
        top_ed++;
    }
    else
    {
...
ROOM_INDEX_DATA *new_room_index( void )
{
    ...
    if ( !room_index_free )
    {
        pRoom           =   alloc_perm( sizeof(*pRoom), PERM_OLC );
        top_room++;
    }
    else
    {
...
AFFECT_DATA *new_affect( void )
{
    ...
    if ( !affect_free )
    {
        pAf             =   alloc_perm( sizeof(*pAf), PERM_OLC );
        top_affect++;
    }
    else
    {
...
SHOP_DATA *new_shop( void )
{
    ...
    if ( !shop_free )
    {
        pShop           =   alloc_perm( sizeof(*pShop), PERM_OLC );
        top_shop++;
    }
    else
    {
...
OBJ_INDEX_DATA *new_obj_index( void )
{
    ...
    if ( !obj_index_free )
    {
        pObj           =   alloc_perm( sizeof(*pObj), PERM_OLC );
        top_obj_index++;
    }
    else
    {
...
MOB_INDEX_DATA *new_mob_index( void )
{
    ...
    if ( !mob_index_free )
    {
        pMob           =   alloc_perm( sizeof(*pMob), PERM_OLC );
        top_mob_index++;
    }
    else
    {
